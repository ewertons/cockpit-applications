...
const context = await esbuild.context({
    ...
    loader: { ".js": "jsx", ".py": "text", ".tsx": "tsx", ".ts": "ts" },
    minify: production,
    nodePaths,
    outdir,
    metafile: true,
...
try {
    ...
    if (!args.watch) {
        ...
        // Write runtime npm modules list
        const runtimeModules = [...bundledPackages].sort().join('\n') + '\n';
        fs.writeFileSync('runtime-npm-modules.txt', runtimeModules);
    }
} catch (e) {
    if (!args.watch)
        process.exit(1);
...
if (args.watch) {
    watch_dirs('src', async (path) => {
        console.log("change detected:", path);
        await context.rebuild().catch(e => true);
    });

    // Rebuild once to start
    await context.rebuild().catch(e => true);
} else {
